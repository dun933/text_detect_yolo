...
for train_label in train_labels:
    for char in train_label["chars"]:
        chars.add(char["transcription"])
print(len(chars))
...
def img_split_to_box(im, splitW=15, adjust=True):
    ...
    均等分割box
    """
    tmpIm = im == 255   # 判断每个元素是否为255，若是则为True
    h, w = tmpIm.shape[:2]
    num = w // splitW + 1 # // 整数除法
...
def resize_img_box(p, scale=416, max_scale=608, splitW=15, adjust=True):
    path = root.format(index_format(p))
    img = cv2.imread(path)
    if img is None:
        return None, []
    points = get_points(train_labels[p]['chars'])
    h, w = img.shape[:2]
...
checkpoint = ModelCheckpoint(filepath='./model.hd5',
monitor='class_loss_loss',
verbose=1,
save_best_only='True',
mode='auto',
period=1)
...
try:
    ...
    pass
tf.keras.models.save_model(textModel,'model.hdf5')